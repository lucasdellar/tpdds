...
public class Indicador {
	...
	public Double aplicarIndicador(String periodo, Empresa unaEmpresa, RepositorioIndicadores repo){
		...
		if(!parser.Parser.chequearQueQuedenSoloNumeros(cuentaLiteralMatematica)) throw new IndicadorInvalidoException("Se esta utilizando un indicador sobre una empresa con datos insuficientes.");
		return parser.Parser.resolverCuenta(cuentaLiteralMatematica);
	}

	/*private void verificarQueNoQuedenPalabras(String cuentaMatematica){
		if (pasarElCheckeoDelParser(cuentaMatematica)) throw new IndicadorInvalidoException("El indicador no esta escrito correctamete. Por favor verifiquelo.");
	}*/
	...
	private String reemplazarPorLosIndicadores(String cuentaMatematica, String periodo, Empresa unaEmpresa){
		...
		for (Indicador indicador : repositorioIndicadores.getIndicadores()) {
			if(cuentaMatematica.contains(indicador.getNombre())) aux = aux.replace(indicador.getNombre(), String.valueOf(indicador.aplicarIndicador(periodo, unaEmpresa, repositorioIndicadores))); 
		}
		return aux;
	}
	...
	private String reemplazarPorLasCuentas(ArrayList<Cuenta> cuentas){
		...
		for (Cuenta cuenta : cuentas) {
			if(getFormula().contains(cuenta.getNombre())) aux = aux.replace(cuenta.getNombre(), cuenta.getValor().toString()); 
		}
		return aux;
	}
